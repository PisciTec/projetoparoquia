<?php
    ...
    ";
      if (isset($_POST["consulta"]) and ($_POST["consulta"])) {
        ...
        $consulta = "select codcliente, nome, cgccpf, cgfrg, nascto from jornal_cliente where (nascto like '$mes') and (ativo = '1') order by nascto asc";
        ...
        <table border=\"0\" cellpadding=\"1\" cellspacing=\"1\" width=\"590\" class=\"tabela_formatada\">
          ...
          while ($linha = mysql_fetch_array($resposta)) {
                  ...
                  $cpf = $linha["cgccpf"];
                  $rg = $linha["cgfrg"];
                  $data_nasc = $linha["nascto"];
                  $dado = explode("-",$data_nasc);
                  $dia = $dado[2];
                  echo "
                  <tr>
                      <td class=\"td2\" width=\"10%\">$codcliente</td>
